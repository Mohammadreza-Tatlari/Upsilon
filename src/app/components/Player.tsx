...
export default function Player({
  ...
  //stops the music when Music Page Component is unmounted
  useEffect(() => {
    return () => {
      Howler.stop()
      Howler.volume(1.0);
      console.log("music is unmounted");
    };
  ...
  return (
    <>
      <div className=" z-50 grid-flow-row fixed bottom-0 w-screen bg-white shadow-md h-auto p-3">
        ...
        <h4 className="flex justify-center items-center lg:justify-start xl:justify-start">{trackDetail?.tracktitle ? `${trackDetail.tracktitle } - ${trackDetail.trackArtist}` : "Music Name"}</h4>
        <div className="relative flex max-lg:justify-center space-x-3">
          <label htmlFor="durationController">Range</label>
          <input
          ...
          />
          <div className="p-2 mx-1">{formattedTime}</div>
